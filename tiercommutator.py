...
def length(A):
    """
    The length of a pauli in qublocks is just log_16(A), e.g. length(15) = 1, length(16) = 2
    :param A: pauli string
    :return: length of pauli string
    """
    if A == 0:
        return 1
    return int(np.log(A) / np.log(16)) + 1
...
def single_convolve(A, B):
    ...
    for i in range(length(A)):
        key, val = commute(A, B * (16 ** i))
        if key is not None:
            if key in res:
                res[key] += val
            else:
                res[key] = val
    for j in range(1, length(B)):  # skip 0 because we already did it
        key, val = commute(A * (16 ** j), B)
        if key is not None:
            if key in res:
                res[key] += val
            else:
...
def cycle(A):
    ...
    for p in permutations:
        ...
        for i in A:
            idx = 0
            for j in range(length(i) * 2 + 1):  # lmao oof
