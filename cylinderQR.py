...
while i < len(space):
    ...
    before = len(space)
    for j in range(min(i + 12, len(space))):
    ...
    # save space as np array in pickle file
    with open('cylinderQR4spaceSKIP.pkl', 'wb') as f1:
        pickle.dump(space, f1)
    with open('cylinderQR4groupSKIP.pkl', 'wb') as f2:
        pickle.dump(group, f2)
