# We will now consider the n = 3 case. There are 2*n = 6 qubits arranged in a circle.
# We have access to cylically symmetric operators, which are invariant under rotations.

# Our vector space of all Hermitian operators has size 4^(2*3) = 4096.
# Lets see if we can generate... all of them. LOL
# The cyclically symmetric subspace has size 16 + 16 * 15 + 16 * 15 * 14 / 3 = 1376.
# Lol.

# Vectors are naturally described as 4096-tuples of real numbers. They are real, because Hermitian operators are real.

# Specifically, the basis vector a * 1 + b* 4 + ... f * 4^5 corresponds to the operator ABCDEF
# on the 6 qubits, where a is in 0...3, corresponding to IXYZ, and so on.

# We begin with the capability to do any operator of the form ABIIII + IIABII + IIIIAB
        ...
        for j in range(num_basis):
            ...
            if flips == 1:
                if phase == 1:
                    ...
                    raise Exception("Flips = 1 but phase = " + str(phase))
                res[i ^ j] += phase * A[i] * B[j]
    return res
